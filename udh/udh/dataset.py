...
class SyntheticDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        # grayscale
        img_a = torch.mean(img_a, dim=0, keepdim=True) #转灰度
        # print(img_a.size())
        ...
        if self.pic_size - self.rho - self.patch_size >= self.rho:
        ...
        return img_a,img_b, patch_a, patch_b, corners.float(), delta.float()   #最后一项用于监督学习，在非监督学习中用不到 #原版没有img_b!
